...
let letterFrequency = {}; // NEW: Track letter frequencies by position
let bigramFrequency = {}; // NEW: Track common letter pairs
let stats = {
    totalRounds: 0,
    totalGuesses: 0
};
let settings = {
    ...
    useSmartFiltering: true // NEW: Enable smart filtering
};
...
function incrementWordFrequency(word) {
    ...
    // NEW: Also track letter frequencies by position
    for (let i = 0; i < word.length; i++) {
        const key = `${i}_${word[i]}`;
        letterFrequency[key] = (letterFrequency[key] || 0) + 1;
    }
    
    // NEW: Track bigrams (letter pairs)
...
// ---------------------------
// Smart Filtering Functions (NEW)
// ---------------------------

// Calculate word score based on frequency and letter patterns
function calculateWordScore(word) {
    let score = 0;
    
    // Word frequency score (0-100)
    const maxFreq = Math.max(...Object.values(wordFrequency), 1);
    const freqScore = ((wordFrequency[word] || 0) / maxFreq) * 100;
    score += freqScore * 0.4; // 40% weight
    
    // Letter position frequency score
    let positionScore = 0;
    for (let i = 0; i < word.length; i++) {
        const key = `${i}_${word[i]}`;
        positionScore += letterFrequency[key] || 0;
    }
    score += (positionScore / word.length) * 0.3; // 30% weight
    
    // Bigram frequency score
    let bigramScore = 0;
    for (let i = 0; i < word.length - 1; i++) {
        const bigram = word[i] + word[i + 1];
        bigramScore += bigramFrequency[bigram] || 0;
    }
    score += (bigramScore / (word.length - 1)) * 0.3; // 30% weight
    
    return score;
}

// Filter by common English letter patterns
function hasCommonPattern(word) {
    // Common vowel patterns
    const hasVowels = /[aeiou]/i.test(word);
    if (!hasVowels && word.length > 3) return false;
    
    // Avoid too many consonants in a row
    const tooManyConsonants = /[bcdfghjklmnpqrstvwxyz]{4,}/i.test(word);
    if (tooManyConsonants) return false;
    
    // Uncommon starting patterns
    const uncommonStarts = /^(bz|dx|fq|gx|jq|kx|qz|vx|wx|zx)/i;
    if (word.length >= 3 && uncommonStarts.test(word)) return false;
    ...
    document.body.insertAdjacentHTML(
        ...
        <div id="bottom-ui">
            ...
            <style>
                ...
                #stats-modal.show .settings-content {
                    opacity: 1;
                    transform: scale(1);
                }
            ...
            <div class="settings-content">
                ...
                <div class="setting-item">
                    <label class="setting-label">
                        ...
                        <span class="info-icon">
                            i
                            <span class="tooltip">Use advanced AI pattern matching to reduce word list from 400+ to 50 best candidates</span>
                        </span>
                    </label>
                    <div class="toggle-container">
                        <div class="toggle-switch active" id="smart-toggle">
                            <div class="toggle-slider"></div>
                        </div>
                        <span class="toggle-label" id="smart-label">ON</span>
                    </div>
                </div>
...
function setupSettingsModal() {
    ...
    const smartToggle = document.getElementById("smart-toggle"); // NEW
    const smartLabel = document.getElementById("smart-label"); // NEW
    ...
    // NEW: Toggle smart filtering
    smartToggle.addEventListener("click", () => {
        const isActive = smartToggle.classList.toggle("active");
        smartLabel.textContent = isActive ? "ON" : "OFF";
    });
    ...
    saveBtn.addEventListener("click", () => {
        ...
        settings.useSmartFiltering = smartToggle.classList.contains("active"); // NEW
...
function renderGuesses(words) {
    ...
    if (settings.sortByFrequency || settings.useSmartFiltering) {
        // Calculate scores for all words
        const scoredWords = sortedWords.map(word => ({
            word,
            score: calculateWordScore(word),
            freq: wordFrequency[word] || 0
        }));
        
        if (settings.sortByFrequency) {
            // Sort by frequency first, then score
        ...
        sortedWords = scoredWords.map(item => item.word);
    ...
    sortedWords.forEach((word, index) => {
        ...
        // Highlight high-confidence words (top 20% or top 10)
        if (settings.useSmartFiltering && 
            (index < Math.min(10, sortedWords.length * 0.2))) {
            btn.style.background = "linear-gradient(135deg, rgba(0, 255, 100, 0.25), rgba(0, 255, 100, 0.15))";
...
function filterHints(words) {
    ...
    let filtered = words.filter((w) => regex.test(w));
    
    // NEW: Apply smart filtering if enabled and there are too many words
    if (filtered.length > 100 && settings.useSmartFiltering) {
        const revealedCount = hints.filter(h => h.textContent !== "_").length;
        const totalLength = hints.length;
        
        // If we have some letters revealed, apply pattern filtering
        if (revealedCount > 0 && revealedCount < totalLength) {
            // Filter by common patterns
            filtered = filtered.filter(hasCommonPattern);
            
            // If still too many, prioritize by score and take top 50
